...
function Show-Usage {
    ...
    Write-Host "  -Location LOC          Azure region. Default: westus2"
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-LogInfo "   az eventgrid namespace client generate-sas-token ``"
    Write-LogInfo "     --resource-group $ResourceGroup ``"
    Write-LogInfo "     --namespace-name `$(az eventgrid namespace list -g $ResourceGroup --query '[0].name' -o tsv) ``"
    Write-LogInfo "     --client-name mqtt_proxy ``"
    Write-LogInfo "     --expiry-time-utc ""2025-12-31T23:59:59Z"""
    Write-Host ""
    Write-LogInfo "2. Update your .env file with the following values:"
    Write-LogInfo "   EVENTGRID_MQTT_HOSTNAME=$eventGridHostname"
    Write-LogInfo "   MQTT_CLIENT_ID=mqtt_proxy"
    Write-LogInfo "   MQTT_USERNAME=mqtt_proxy"
    ...
    Write-Host ""
    Write-LogInfo "4. Start the on-premises services:"
